	...
	ejecutar: async (client) => {
		try {
			...
			for (const jugador of datosElo) {
				...
				// Solo agregar cambios si existen roles añadidos o removidos
			...
			if (cambios.length > 0) {
				...
				for (const cambio of cambios) {
					const rolesAñadidos = cambio.agregados.length > 0 ? cambio.agregados.join(', ') : 'Ninguno';
					const rolesRemovidos = cambio.removidos.length > 0 ? cambio.removidos.join(', ') : 'Ninguno';
